...
class Patterns
{
    static void Main()
    {
        ...
        bool globallyFoundCorrectPatter = false;

        for (int patterntStartX = 0; patterntStartX <= matrix.GetLength(0) - pattern.GetLength(0); patterntStartX++)
        {
            for (int  patternStartY= 0; patternStartY <= matrix.GetLength(1) - pattern.GetLength(1) ; patternStartY++)
            {
                ...
                {
                    globallyFoundCorrectPatter = true;
                    long sum = 0;
        ...
        if (globallyFoundCorrectPatter )
        {
            Console.WriteLine("YES {0}",maxSum);
        }
        else
        {
            Console.WriteLine("NO {0}",SumDiagonals(matrix));
        }

    }
    private static long SumDiagonals(int[,] matrix)
    {
        long sum = 0;
